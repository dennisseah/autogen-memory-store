...
from autogen_memory_store.services.memory_store import (
    MemoryStore,
)
...
def mock_service(mocker: MockerFixture) -> MemoryStore:
    ...
    mocker.patch(
        "autogen_memory_store.services.memory_store.DefaultAzureCredential",
        return_value=mock_cred,
    )
    mock_redis = MagicMock()
    mocker.patch(
        "autogen_memory_store.services.memory_store.Redis",
...
def test_read_graph_memory(mock_service: MemoryStore):
    mock_service.redis_client.get = MagicMock(return_value=json.dumps({"key": "value"}))
    assert mock_service.read_graph_memory("test_key") == {"key": "value"}
...
def test_write_graph_memory(mock_service: MemoryStore):
    mock_service.redis_client.set = MagicMock()
    mock_service.write_graph_memory("test_key", {"key": "value"})
    mock_service.redis_client.set.assert_called_once()
...
def test_restore(mock_service: MemoryStore):
    ...
    assert restored_memory is None


def test_restore_data(mock_service: MemoryStore):
    mock_service.read_graph_memory = MagicMock(return_value=MagicMock())
